...
std::vector<std::vector<float> > free_markers_positions;
                       ...
                       const std::string& multicast_ip)
{
    ...
    while(ros::ok())
    {
        ...
        do
        {
            ...
            if( numBytes > 0 )
            {
//                ROS_INFO_STREAM("processMocapData: Number of bytes received are: " << numBytes);
//                ROS_WARN("**********************************");
                ...
                if (header == NAT_FRAMEOFDATA && version)
                {
                    ...
                    format.parse();
                    free_markers_positions = format.get_free_markers_positions();

//                    for(size_t i = 0; i < free_markers_positions.size(); i++){
//                        for(size_t j = 0; j < free_markers_positions[i].size(); j++)
//                            ROS_INFO_STREAM("For individual marker no: " << i << " The coordinate: " << j << " is: " <<
//                                            free_markers_positions[i][j]);
//                    }
